        ...
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          >
            {navItems.map((item) => (              <motion.button
                key={item.name}
                onClick={() => scrollToSection(item.href)}                className={`relative px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium transition-all duration-300 rounded-lg cursor-pointer ${
                  activeSection === item.href.slice(1)
                    ? "text-white bg-blue-500 shadow-md"
                    : theme === 'dark'
                      ? "text-gray-200 hover:text-blue-400 hover:bg-gray-700/50"
                      : "text-gray-800 hover:text-blue-600 hover:bg-gray-100/70"
                }`}
            ...
            <div className="flex-1 flex justify-end">            <motion.button
              ...
              className={`p-2 sm:p-3 rounded-lg border transition-all duration-300 shadow-md cursor-pointer ${
                theme === 'dark' 
                  ? 'bg-gray-800/90 border-gray-600/50 text-gray-200 hover:bg-gray-700 hover:text-white'
                  : 'bg-white/98 border-gray-200/60 text-gray-800 hover:bg-gray-50 hover:text-gray-900'
              }`}
              style={{
                backgroundColor: theme === 'dark' ? undefined : '#ffffff',
                borderColor: theme === 'dark' ? undefined : '#e5e7eb'
              }}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="md:hidden"
            >              <div className={`px-3 pt-3 pb-4 space-y-2 backdrop-blur-md rounded-lg mt-2 border shadow-xl ${
                theme === 'dark'
